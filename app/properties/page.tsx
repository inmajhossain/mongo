    ...
    <div className="px-4 sm:px-6 lg:px-8 py-12 min-h-screen">
      <div className="mx-auto max-w-7xl">
        {properties.length === 0 ? (
          ...
          <div className="gap-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid-wrapper">
            {properties.map((property) => (
              <div key={property._id as unknown as string} className="bg-slate-100 shadow-lg hover:shadow-xl mx-auto border-2 border-amber-200 rounded-lg w-full overflow-hidden transition duration-200">
                {/* Image Section */}
                <div className="relative p-1 w-full h-48">
                  <Image
                    src={property.imageUrl || '/default-property.jpg'}
                    alt={property.title}
                    width={500}
                    height={500}
                    className="rounded-t-sm w-full h-50 object-cover"
                ...
                <div className="p-6 text-left">
                  ...
                  <div className="space-y-1">
                    <p className="font-bold text-blue-600 text-2xl">BDT <span>{""}</span> {property.price.toLocaleString()}</p>
                    ...
                    <button
                      ...
                      className="bg-green-800 hover:bg-green-700 mt-4 px-4 py-2 rounded-md w-full font-medium text-white transition duration-200"
                    >
                      Book Now For This Property
                    </button>
                  </div>
                </div>
              </div>
